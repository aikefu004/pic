(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
    (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
    m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
})(window,document,'script','//www.google-analytics.com/analytics.js','ga');
ga('create', 'UA-40716497-7', 'auto');
ga('send', 'pageview');

document.writeln('<div class="header">\n' +
    '    <div class="header__main">\n' +
    '        <div class="brand">\n' +
    '            <div class="brand__logo">\n' +
    '                <a href="/">\n' +
    '                    <img src='+cdn_static+'/img/logo.png" alt="photo editor"/>\n' +
    '                </a>\n' +
    '            </div>\n' +
    '        </div>\n' +
    '        <div class="header__nav">\n' +
    '            <div class="main-menu" id="menuBig">\n' +
    '                <ul class="nav">\n' +
    '                    <li><a class="nav__item--active" href="'+editHref+'">'+editTitle+'</a></li>\n' +
    '                    <li class="dropdown" id="menuSmall">\n' +
    '                        <span class="nav__icon"><i class="ico ico--tools"></i></span>\n' +
    '                        <span>'+editTitleAll+'<i class="ico ico--down"></i></span>\n' +
    '                        <ul>\n' +
    '                            <li><a class="nav__item--active" href="'+editHref+'"><i class="ico ico--edit"></i>'+editTitle+'</a></li>\n' +
    '                        </ul>\n' +
    '                    </li>\n' +
    '                </ul>\n' +
    '            </div>\n' +
    '        </div>\n' +
    '        <div class="top-menu">\n' +
    '            <ul class="nav">\n' +
    '                <li class="nav__item nav__item--sub">\n' +
    '                    <i class="ico ico--hamburger"></i>\n' +
    '                    <ul>\n' +
    '                        <li><a href="/"><i class="ico ico--iloveimg"></i>'+editNavTitle+'</a></li>\n' +
    '                        <li class="divider"></li>\n' +
    '                        <li class="nav__item--sub"><a href="#"><i class="ico ico--lang"></i>'+editNavLanguage+'</a>\n' +
    '                            <ul class="lang__menu">\n' +
    '                                <li><a href="/en"> English</a></li>\n' +
    '                                <li><a href="/es"> Español</a></li>\n' +
    '                                <li><a href="/fr"> Français</a></li>\n' +
    '                                <li><a href="/de"> Deutsch</a></li>\n' +
    '                                <li><a href="/it"> Italiano</a></li>\n' +
    '                                <li><a href="/pt"> Português</a></li>\n' +
    '                                <li><a href="/ja"> 日本語</a></li>\n' +
    '                                <li><a href="/ru"> Pусский</a></li>\n' +
    '                                <li><a href="/ko"> 한국어</a></li>\n' +
    '                                <li><a href="/zh-cn"> 中文 (简体)</a></li>\n' +
    '                                <li><a href="/zh-tw"> 中文 (繁體)</a></li>\n' +
    '                                <li><a href="/ar"> العربية</a></li>\n' +
    '                                <li><a href="/bg"> Български</a></li>\n' +
    '                                <li><a href="/ca"> Català</a></li>\n' +
    '                                <li><a href="/nl"> Dutch</a></li>\n' +
    '                                <li><a href="/el"> Ελληνικά</a></li>\n' +
    '                                <li><a href="/hi"> हिन्दी</a></li>\n' +
    '                                <li><a href="/id"> Bahasa Indonesia</a></li>\n' +
    '                                <li><a href="/ms"> Bahasa Melayu</a></li>\n' +
    '                                <li><a href="/pl"> Polski</a></li>\n' +
    '                                <li><a href="/sv"> Svenska</a></li>\n' +
    '                                <li><a href="/th"> ภาษาไทย</a></li>\n' +
    '                                <li><a href="/tr"> Türkçe</a></li>\n' +
    '                                <li><a href="/uk"> Українська</a></li>\n' +
    '                                <li><a href="/vi"> Tiếng Việt</a></li>\n' +
    '                            </ul>\n' +
    '                        </li>\n' +
    '                    </ul>\n' +
    '                </li>\n' +
    '            </ul>\n' +
    '        </div>\n' +
    '    </div>\n' +
    '</div>');